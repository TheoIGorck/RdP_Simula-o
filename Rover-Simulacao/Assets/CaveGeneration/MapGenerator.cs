...
public class MapGenerator : MonoBehaviour {
    ...
    private void Awake()
    {
        ...
        //Dijkstra((int)_from.transform.position.x, (int)_from.transform.position.z);
    }
  ...
  public void Draw()
   ...
   public void ChecarColisao(int x, int y)
    ...
    {
        //Vector2 destination = new Vector2(_to.transform.position.x, _to.transform.position.y);
        foreach(Vector2 originPos in _prevPos.Keys)
        {
            foreach(Vector2 pos in _prevPos.Values)
            {
                Gizmos.color = Color.green;
                //Gizmos.DrawCube(new Vector3(pos.x + 0.5f, 0.5f, pos.y + 0.5f), Vector3.one);
            }
        }
    }*/
    ...
    private void OnDrawGizmos()
    {
        if(map != null && _canFill)
        {
            for(int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        Gizmos.color = Color.blue;
                        Gizmos.DrawCube(new Vector3(x + 0.5f, 0.1f, y + 0.5f), Vector3.one);
                    }
                    if (_costMap[x, y] == 2)
                    {
                        Gizmos.color = Color.green;
        ...
        foreach(Vector2 position in _finishedPath)
        {
            Gizmos.color = Color.black;
            Gizmos.DrawCube(new Vector3(position.x + 0.5f, 5f, position.y + 0.5f), Vector3.one);
        }
    }
}
